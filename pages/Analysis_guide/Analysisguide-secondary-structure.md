...
permalink: Analysisguide-secondary-structure.html
folder: Analysis_guide
---

<link rel="stylesheet" href="css/theme-purple.css">

Secondary structure analysis can identify changes in secondary structure across
a simulation or differences between systems.
There are several types of secondary structure, including alpha helices,
beta sheets, turns, and loops.

`cpptraj` has a keyword for secondary structure--`secstruct`.
If your system has 350 protein residues, the command to use would be:
```
secstruct :1-350 out WT_protein_system_secstruct.gnu
```
This creates a `gnuplot` file for plotting the secondary structure (color) for
each residue (y-axis) across time (x-axis).
The file that is generated by `cpptraj`, however, is not ideal and may
crash `gnuplot`.
That initial `gnuplot` file can be modified by using a second script,
`secstruct-gnu-fix.sh`.
